    ...
    <div>
      ...
      <v-card class="container-carrinho">
        <div class="listagemProdutosContainer col">
          ...
          <input type="button" value="Voltar ao Histórico" class="submit-btn elementos-cartao"
                 @click="redirecionaAoHistorico()">
        </div>
      </v-card>
    </div>
  </div>
...
export default defineComponent({
  ...
  methods: {
    async preencheCampos() {
      const response = await axios.get('http://localhost:8081/pedidos/buscar/' + this.pedidoId);
      this.pedido = response.data;
    },
    redirecionaAoHistorico() {
